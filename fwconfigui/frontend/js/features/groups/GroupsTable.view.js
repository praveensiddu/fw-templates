function GroupsTableView({ env, rows, filters, setFilters, onAdd, onCheckUsed, onShowUsedInGroups, usedInGrpModal, setUsedInGrpModal, onEdit, onDelete, onExclude, onExcludeEnvCommon, editingKey, draft, setDraft, memberOptions, canSubmit, onCancelEdit, onSave }) {
          ...
          >
            {(() => {
              ...
              const currentRow = !isNew
                ? (rows || []).find((r) => `${safeTrim(r.filename) || "groups.yaml"}::${r.name}` === editingKey)
                : null;
              ...
              return (
                <>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 12 }}>
                    <div style={{ fontWeight: 600, fontSize: 16 }}>{isNew ? "Add group" : `Edit group: ${safeTrim(currentRow?.name)}`}</div>
                  ...
                  <div style={{ display: "grid", gap: 10 }}>
                    ...
                    <div>
                      <div className="muted" style={{ marginBottom: 4 }}>members</div>
                      <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                        <MultiSelectPicker
                          ...
                          inputTestId="groups-panel-members"
                          onEnter={onSave}
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input className="filterInput" value={draft.filename} onChange={(e) => setDraft((p) => ({ ...p, filename: normalizeFilename(e.target.value) }))} placeholder="groups.yaml" />
                    </div>
                  </div>
        ...
        <tbody>
          {rows.map((r, idx) =>
            (() => {
              ...
              const isFmExtract = safeTrim(r.filename) === "fm_extract_groups.yaml";
