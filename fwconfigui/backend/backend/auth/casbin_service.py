...
def enforce_rbac(
    *,
    ...
    Args:
        ...
        usercontext: User context with username, roles, groups, product_roles
        obj: Resource path (e.g., "/products/product1")
        act: HTTP method (e.g., "GET", "POST", "PUT", "DELETE")
        product: Optional product context with id field
    ...
    product_roles = usercontext.get("product_roles", {})
    ...
    with _ENFORCER_LOCK:
        ...
        allowed = enforcer.enforce(usercontext, obj, act, product_ctx)

    if not allowed:
        # Log security event for audit trail
        logger.warning(
            "RBAC: Access denied - user=%s, roles=%s, groups=%s, product_roles=%s, resource=%s, action=%s, product_id=%s",
            username,
            roles,
            groups,
            product_roles,
