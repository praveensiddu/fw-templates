...
def enforce_request(usercontext: dict[str, Any], obj: str, act: str, product: dict[str, Any] | None = None) -> None:
    enforce_rbac(enforcer=_ENFORCER, usercontext=usercontext, obj=_normalize_obj(obj), act=act, product=product)


def check_permission(usercontext: dict[str, Any], obj: str, act: str, product: dict[str, Any] | None = None) -> bool:
    """Check if user has permission without raising exception.

    Args:
        usercontext: User context dict with roles and groups
        obj: Object path (e.g. "/products/product1")
        act: Action (e.g. "GET", "PUT", "DELETE")
        product: product context dict with id

    Returns:
        True if user has permission, False otherwise
    """
    product_ctx = product or {"id": ""}
    return _ENFORCER.enforce(usercontext, _normalize_obj(obj), act, product_ctx)
...
def calculate_resource_permissions(
    ...
    Args:
        user_context: User context dict with roles, groups, product_roles
        resource_path: The resource path (e.g., "/products/{productname}")
        product_context: Optional product context with id for product-scoped permissions
    ...
    """
    product_ctx = product_context or {"id": ""}

    can_view = any(
        check_permission(user_context, resource_path, action, product_ctx)
...
def add_permissions_to_items(
    ...
    Args:
        ...
        resource_path_template: Path template with {item_id} placeholder (e.g., "/products/{item_id}")
        product_id: productlication ID for product-scoped resources
    """
    for item_id, item_data in items.items():
        product_context = {"id": product_id or item_id}
        resource_path = resource_path_template.format(item_id=item_id)
        item_data["permissions"] = calculate_resource_permissions(
            user_context, resource_path, product_context
        )
...
def wrap_response_with_permissions(
    ...
    """
    permissions = calculate_resource_permissions(user_context, resource_path, product_context)
    return {
        data_key: data,
...
def get_user_context(user_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "groups": groups,
        "roles": roles,
        "product_roles": rolemgmtimpl.get_product_roles(groups, user_id),
    }
...
def require_rbac(
    *,
    ...
    product_id: str | Callable[[Request], str] | None = None,
    ...
    def _dep(request: Request, user_context: dict[str, Any] = Depends(get_current_user_context)) -> None:
        ...
        if product_id is not None:
            resolved_product_id = product_id(request) if callable(product_id) else str(product_id)
        enforce_request(user_context, resolved_obj, resolved_act, {"id": resolved_product_id})
